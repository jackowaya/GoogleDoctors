...
foreach my $docRef (@doctorInfo) {
	...
	$filename = "$outfolder/$doc{'serial'}.1.html";
	...
	$filename = "$outfolder/$doc{'serial'}.2.html";
	...
	getSubLinks($filename, $outfolder, "$doc{'serial'}.2");
...
sub generateSearchUri {
	...
	if ($page == 2) {
		$uri-> query_form(ie => "UTF-8", q => "$doc{'lastname'} $doc{'firstname'} $doc{'specialty'} $doc{'city'}", start => "10");
	} else {
		$uri-> query_form(ie => "UTF-8", q => "$doc{'lastname'} $doc{'firstname'} $doc{'specialty'} $doc{'city'}");
	}
	return $uri;
}
